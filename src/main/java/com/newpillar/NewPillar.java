...
import com.newpillar.integration.MessageServiceIntegration;
...
public class NewPillar extends JavaPlugin {
   ...
   private com.newpillar.reward.RewardManager rewardManager;

   public void onEnable() {
      instance = this;
      
      // 初始化调度器工具类（必须在其他组件之前初始化）
      ...
      // 初始化MessageService集成（直接集成，用于跨服消息）
      this.messageServiceIntegration = new MessageServiceIntegration(this);
      this.messageServiceIntegration.initialize();
      
      // 初始化奖励管理器（负责奖励消息和招人）
      this.rewardManager = new com.newpillar.reward.RewardManager(this);
      ...
      // 注册奖励查询指令
      this.getCommand("reward").setExecutor(new com.newpillar.commands.RewardCommand(this));
      ...
      this.getLogger().info("MessageService跨服消息集成: " + (this.messageServiceIntegration.isEnabled() ? "已启用" : "未启用（插件缺失）"));
   }
   ...
   public MessageServiceIntegration getMessageServiceIntegration() {
      return this.messageServiceIntegration;
   }

   public com.newpillar.reward.RewardManager getRewardManager() {

...
public class NewPillarCommand implements CommandExecutor, TabCompleter {
         ...
         if (args.length == 0) {
            ...
            switch (var7) {
               ...
               case "hook":
                  ...
                  this.handleHookCommand(player, args);
                  break;
               default:
                  this.sendHelp(player);
            }
   ...
   public List<String> onTabComplete(CommandSender sender, Command command, String alias, String[] args) {
      if (args.length == 1) {
         ...
         if (sender.hasPermission("newpillar.admin")) {
            completions.addAll(Arrays.asList("start", "forcestart", "stop", "test", "mapmenu", "event", "item", "loottest", "autostart", "menu", "minplayers", "allitems", "collapse", "hook"));
         }
         ...
         return completions;
      } else if (args.length == 2 && args[0].equalsIgnoreCase("hook")) {
         // hook 指令的自动补全
   ...
   private void sendHelp(Player player) {
      ...
      if (player.hasPermission("newpillar.admin")) {
         ...
         player.sendMessage("§f/np hook §7- 手动触发插件挂钩初始化 (Admin)");
         player.sendMessage("§f/np hook status §7- 查看插件挂钩状态 (Admin)");
      }
   }
    ...
    * 处理插件挂钩命令
    */
   private void handleHookCommand(Player player, String[] args) {
      if (args.length >= 2 && args[1].equalsIgnoreCase("status")) {
         // 显示插件挂钩状态
         player.sendMessage("§6=== 插件挂钩状态 ===");
         
         // MessageService 状态
         var messageService = plugin.getMessageServiceIntegration();
         if (messageService != null) {
            boolean enabled = messageService.isEnabled();
            player.sendMessage("§fMessageService: " + (enabled ? "§a已连接" : "§c未连接"));
            if (enabled) {
               player.sendMessage("§7  当前服务器: §f" + plugin.getConfig().getString("cross-server.server-name", "unknown"));
               player.sendMessage("§7  目标服务器: §f" + plugin.getConfig().getStringList("cross-server.target-servers"));
            }
         } else {
            player.sendMessage("§fMessageService: §c未初始化");
         }
         
         // ThwReward 状态
         var thwReward = plugin.getThwRewardIntegration();
         if (thwReward != null) {
            boolean enabled = thwReward.isEnabled();
            player.sendMessage("§fThwReward: " + (enabled ? "§a已连接" : "§c未连接"));
         } else {
            player.sendMessage("§fThwReward: §c未初始化");
         }
         
         // 数据库状态
         var dbManager = plugin.getDatabaseManager();
         if (dbManager != null && dbManager.isConnected()) {
            player.sendMessage("§f数据库: §a已连接");
         } else {
            player.sendMessage("§f数据库: §c未连接");
         }
         
         player.sendMessage("§7使用 §f/np hook §7手动触发初始化");
         return;
      }
      
      // 手动触发初始化
      player.sendMessage("§6正在手动触发插件挂钩初始化...");
      
      // 重新初始化 MessageService
      var messageService = plugin.getMessageServiceIntegration();
      if (messageService != null) {
         player.sendMessage("§7重新初始化 MessageService...");
         messageService.initialize();
         if (messageService.isEnabled()) {
            player.sendMessage("§aMessageService 初始化成功！");
         } else {
            player.sendMessage("§cMessageService 初始化失败，请检查插件是否已安装");
         }
      }
      ...
      var thwReward = plugin.getThwRewardIntegration();
      if (thwReward != null) {
         player.sendMessage("§7重新初始化 ThwReward...");
         thwReward.initialize();
         if (thwReward.isEnabled()) {
            player.sendMessage("§aThwReward 初始化成功！");
         } else {
            player.sendMessage("§cThwReward 初始化失败，请检查插件是否已安装");
         }
      }
      
      player.sendMessage("§6插件挂钩初始化完成！");
      player.sendMessage("§7使用 §f/np hook status §7查看详细状态");
   }
}

...
import com.newpillar.utils.MessageUtils;
...
public class ItemEffectManager {
    ...
    // 蓝屏效果跟踪: 玩家UUID -> [移动modifierUUID, 跳跃modifierUUID]
    private final Map<UUID, UUID[]> blueScreenEffects = new HashMap<>();
    
    // 蓝屏效果玩家集合: 用于事件监听取消移动和跳跃
    private final Set<UUID> blueScreenPlayers = new HashSet<>();
    ...
    private void initCooldownConfig() {
        ...
        // 让你飞起来 - 15秒冷却
        cooldownTimes.put(SpecialItemManager.SpecialItemType.FLY_MACE, 15000L);
    ...
    public void startCooldownUpdateTask() {
        ...
        cooldownUpdateTask = SchedulerUtils.runTimerGlobal(1L, 1L, task -> {
            // 更新所有玩家的BossBar冷却显示
    ...
    private boolean checkItemRestrictions(Player player, SpecialItemManager.SpecialItemType type) {
        ...
        if (cooldownSeconds != null && cooldownSeconds > 0 && !managesOwnCooldown(type)) {
            if (plugin.getItemCooldownManager().isOnCooldown(player, itemId, cooldownSeconds)) {
                ...
                MessageUtils.sendPlainMessage(player, ChatColor.RED + "「" + itemName + "」冷却中，剩余 " + remaining + " 秒");
                return false;
            }
        }
        ...
        if (requiresTargetPlayer(type) && !managesOwnTarget(type)) {
            ...
            if (target == null) {
                MessageUtils.sendPlainMessage(player, ChatColor.RED + "附近没有其他玩家！无法使用" + itemName + "。");
                return false;
            }
        }
    ...
    public void onPlayerInteract(Player player, ItemStack item, org.bukkit.block.Block clickedBlock) {
        ...
        switch (type) {
            ...
            case FLY_MACE -> {
                useFlyMace(player);
                setCooldown(player, SpecialItemManager.SpecialItemType.FLY_MACE);
            ...
            case RUSSIAN_ROULETTE -> {
                ...
                if (plugin.getRussianRouletteGUI().isLoaded(mainHand)) {
                    // 已装填，执行开枪（fire方法内部已处理物品消耗）
                    plugin.getRussianRouletteGUI().fire(player, mainHand);
                    shouldConsume = false; // fire方法内部已消耗物品，外部不再消耗
                } else {
                    // 未装填，打开选择界面（不消耗物品）
                    useRussianRoulette(player);
                    shouldConsume = false; // 打开界面不消耗物品
                }
    ...
    private void useSpear(Player player) {
        // 长矛：突进效果 - 向前冲刺
        MessageUtils.sendPlainMessage(player, ChatColor.GOLD + "突进！");
    ...
    private boolean useKnockbackStick(Player player) {
        // 击退棒：仅作为普通武器使用，右键无特殊效果
        // 击退效果由物品本身的附魔提供
        return false; // 返回false表示不消耗物品且不触发使用成功逻辑
    }
    ...
    private void useBone(Player player) {
        ...
        MessageUtils.sendPlainMessage(player, ChatColor.GOLD + "嘎嘣脆！");
    }
    ...
    private boolean useCard(Player player) {
        ...
        if (!hasTarget) {
            MessageUtils.sendPlainMessage(player, ChatColor.RED + "附近没有其他玩家！无法使用牌。");
            return false;
        }

        Bukkit.broadcastMessage(player.getName() + ": " + ChatColor.YELLOW + "我要验牌！");
        player.getWorld().playSound(player.getLocation(), Sound.ENTITY_PLAYER_ATTACK_SWEEP, 1.0f, 1.0f);
        ...
        for (Entity entity : player.getNearbyEntities(5, 5, 5)) {
            if (entity instanceof Player && !entity.equals(player)) {
                ...
                SchedulerUtils.runOnEntity(entity, () -> {
                    entity.setVelocity(knockback);
                });
                MessageUtils.sendPlainMessage(targetPlayer, ChatColor.RED + "被" + player.getName() + "验牌击退！");
            }
        }
        return true;
    }
    ...
    private void applyFeatherEffect(Player player) {
        ...
        // 使用玩家的 EntityScheduler，这样无论玩家移动到哪个区域都能正确执行
        ...
        }, null, GameConstants.DELAY_IMMEDIATE, GameConstants.TICKS_PER_SECOND);
    ...
    private boolean usePixie(Player player) {
        ...
        if (!hasTarget) {
            MessageUtils.sendPlainMessage(player, ChatColor.RED + "附近没有其他玩家！无法使用皮鞋。");
            return false;
        }
        ...
        // 皮鞋：给我擦皮鞋！
        Bukkit.broadcastMessage(player.getName() + ": 给我擦皮鞋！");
    ...
    private boolean useBlueScreen(Player player) {
        ...
        if (target == null) {
            // 没有有效目标，拒绝使用
            MessageUtils.sendPlainMessage(player, ChatColor.RED + "附近没有其他玩家！无法使用蓝屏。");
            return false;
        }
        ...
        if (plugin.getItemCooldownManager().isOnCooldown(player, itemId, cooldownSeconds)) {
            long remaining = plugin.getItemCooldownManager().getRemainingCooldown(player, itemId, cooldownSeconds);
            MessageUtils.sendPlainMessage(player, ChatColor.RED + "「蓝屏」冷却中，剩余 " + remaining + " 秒");
            return false;
        }
...

        {
            ...
            MessageUtils.sendPlainMessage(player, ChatColor.BLUE + "对 " + target.getName() + " 使用蓝屏！");
            MessageUtils.sendPlainMessage(target, ChatColor.BLUE + "" + ChatColor.BOLD + "╔════════════════════════════════╗");
            MessageUtils.sendPlainMessage(target, ChatColor.BLUE + "" + ChatColor.BOLD + "║      WINDOWS ERROR             ║");
            MessageUtils.sendPlainMessage(target, ChatColor.BLUE + "" + ChatColor.BOLD + "║                                ║");
            MessageUtils.sendPlainMessage(target, ChatColor.BLUE + "" + ChatColor.BOLD + "║  :( Your PC ran into a problem ║");
            MessageUtils.sendPlainMessage(target, ChatColor.BLUE + "" + ChatColor.BOLD + "║  and needs to restart.         ║");
            MessageUtils.sendPlainMessage(target, ChatColor.BLUE + "" + ChatColor.BOLD + "╚════════════════════════════════╝");

            // 蓝屏效果：12秒内无法移动、无法跳跃、失明、无法挖掘、无法攻击
            // 先清除可能存在的旧蓝屏效果（包括取消旧的任务）
            this.clearBlueScreenEffect(target);
            // 取消可能存在的旧蓝屏定时任务
            ScheduledTask oldTask = activeEffects.remove(target.getUniqueId());
            if (oldTask != null) {
                oldTask.cancel();
            }

            // 将玩家添加到蓝屏效果集合中（用于事件监听）
            blueScreenPlayers.add(target.getUniqueId());

            // 记录蓝屏效果添加的属性修改器UUID，用于后续清理
            UUID moveModifierUUID = UUID.randomUUID();
            UUID jumpModifierUUID = UUID.randomUUID();
            blueScreenEffects.put(target.getUniqueId(), new UUID[]{moveModifierUUID, jumpModifierUUID});

            // 添加无法移动效果（将移动速度设为0）
            AttributeInstance speedAttr = target.getAttribute(Attribute.MOVEMENT_SPEED);
            if (speedAttr != null) {
                // 使用ADD_NUMBER将移动速度设为0（基础值0.1 - 0.1 = 0）
                speedAttr.addModifier(new AttributeModifier(moveModifierUUID, "blue_screen_move", -GameConstants.BASE_MOVEMENT_SPEED, Operation.ADD_NUMBER));
            }

            // 添加无法跳跃效果（将跳跃力度设为0）
            AttributeInstance jumpAttr = target.getAttribute(Attribute.JUMP_STRENGTH);
            if (jumpAttr != null) {
                // 使用ADD_NUMBER将跳跃力度设为0（基础值0.42 - 0.42 = 0）
                jumpAttr.addModifier(new AttributeModifier(jumpModifierUUID, "blue_screen_jump", -GameConstants.BASE_JUMP_STRENGTH, Operation.ADD_NUMBER));
            }

            // 挖掘疲劳（无法挖掘）- 持续时间12秒（240 ticks）
            ...
            // 使用GlobalScheduler避免玩家被传送后区域调度器无法执行的问题
            ScheduledTask blueScreenTask = SchedulerUtils.runLaterGlobal(GameConstants.DELAY_VERY_LONG, task -> {
                // 清除蓝屏效果
                Player onlineTarget = Bukkit.getPlayer(finalTargetUUID);
                if (onlineTarget != null && onlineTarget.isOnline()) {
                    this.clearBlueScreenEffect(onlineTarget);
                    MessageUtils.sendPlainMessage(onlineTarget, ChatColor.GREEN + "系统已恢复！");
                    onlineTarget.playSound(onlineTarget.getLocation(), Sound.BLOCK_NOTE_BLOCK_PLING, 1.0f, 2.0f);
                } else {
                    // 玩家离线，仅从集合中移除
                    blueScreenPlayers.remove(finalTargetUUID);
                    blueScreenEffects.remove(finalTargetUUID);
                }
                // 从活跃效果中移除
                activeEffects.remove(finalTargetUUID);
            });
            // 记录任务以便后续取消
            activeEffects.put(target.getUniqueId(), blueScreenTask);
        }
        return true;
    }
    ...
    private void useHongbao(Player player) {
        // 红包：随机给予金币或物品
        MessageUtils.sendPlainMessage(player, ChatColor.GOLD + "新年快乐！红包打开！");
        ...
        switch (reward) {
            case 0 -> {
                MessageUtils.sendPlainMessage(player, ChatColor.GREEN + "恭喜发财！获得一些经验！");
                player.giveExp(100);
            }
            case 1 -> {
                MessageUtils.sendPlainMessage(player, ChatColor.GREEN + "大吉大利！获得一些食物！");
                player.getInventory().addItem(new ItemStack(Material.COOKED_BEEF, 5));
            }
            case 2 -> {
                MessageUtils.sendPlainMessage(player, ChatColor.GREEN + "万事如意！获得一些资源！");
    ...
    private boolean useHypnosisApp(Player player) {
        ...
        if (targets.isEmpty()) {
            MessageUtils.sendPlainMessage(player, ChatColor.RED + "附近没有其他玩家！无法使用催眠APP。");
            return false;
        }
        ...
        MessageUtils.sendPlainMessage(player, ChatColor.LIGHT_PURPLE + "对 " + target.getName() + " 使用催眠APP！");
        MessageUtils.sendPlainMessage(target, ChatColor.LIGHT_PURPLE + "你被 " + player.getName() + " 催眠了！");
    ...
    private void useBruce(Player player) {
        ...
        MessageUtils.sendPlainMessage(player, ChatColor.GRAY + "召唤布鲁斯！");
    ...
    private void useFlyMace(Player player) {
        ...
        MessageUtils.sendPlainMessage(player, ChatColor.AQUA + "起飞！");
    ...
    public void cleanup() {
        ...
        // 清理所有蓝屏效果（包括属性修改器和药水效果）
        for (UUID playerUUID : new HashSet<>(blueScreenPlayers)) {
            Player player = Bukkit.getPlayer(playerUUID);
            if (player != null && player.isOnline()) {
                // 清理属性修改器
                UUID[] modifierUUIDs = blueScreenEffects.get(playerUUID);
                if (modifierUUIDs != null && modifierUUIDs.length == 2) {
                    AttributeInstance speedAttr = player.getAttribute(Attribute.MOVEMENT_SPEED);
                    if (speedAttr != null) {
                        speedAttr.getModifiers().stream()
                            .filter(mod -> mod.getUniqueId().equals(modifierUUIDs[0]) || mod.getName().equals("blue_screen_move"))
                    ...
                    if (jumpAttr != null) {
                        jumpAttr.getModifiers().stream()
                            .filter(mod -> mod.getUniqueId().equals(modifierUUIDs[1]) || mod.getName().equals("blue_screen_jump"))
        ...
        blueScreenPlayers.clear();
        blueScreenEffects.clear();
    }
     ...
     * @param player 目标玩家
     */
    private void clearBlueScreenEffect(Player player) {
        UUID playerUUID = player.getUniqueId();
        
        // 从集合中移除
        blueScreenPlayers.remove(playerUUID);
        
        // 获取并清理属性修改器
        UUID[] modifierUUIDs = blueScreenEffects.remove(playerUUID);
        if (modifierUUIDs != null && modifierUUIDs.length == 2) {
            // 清理移动速度修改器
            AttributeInstance speedAttr = player.getAttribute(Attribute.MOVEMENT_SPEED);
            if (speedAttr != null) {
     ...
     * @return 是否在蓝屏效果中
     */
    public boolean isBlueScreenPlayer(Player player) {
        return blueScreenPlayers.contains(player.getUniqueId());
    }
    ...
    private boolean useTheWorld(Player player) {
        ...
        if (!hasTarget) {
            MessageUtils.sendPlainMessage(player, ChatColor.RED + "附近没有其他玩家！无法使用砸瓦鲁多。");
            return false;
        }

        MessageUtils.sendPlainMessage(player, ChatColor.GOLD + "The World!!!");
        player.getWorld().playSound(player.getLocation(), Sound.BLOCK_BELL_USE, 1.0f, 1.0f);

        // 冻结周围10格内的所有玩家9秒（180 ticks）
        for (Entity entity : player.getNearbyEntities(10, 10, 10)) {
            if (entity instanceof Player target && !target.equals(player)) {
                MessageUtils.sendPlainMessage(target, ChatColor.GRAY + "时间停止了...");
                ...
                AttributeModifier jumpModifier = new AttributeModifier(
                    jumpModifierUUID,
                    "the_world_jump",
                    -GameConstants.BASE_JUMP_STRENGTH, // 将跳跃力度降为0
                ...
                lockTask[0] = SchedulerUtils.runTimerGlobal(1L, 1L, task -> {
                    if (!target.isOnline()) {
                        SchedulerUtils.cancel(task);
                        return;
                    }
                    // 强制玩家位置和朝向
                    SchedulerUtils.runOnLocation(target.getLocation(), () -> {
                        if (target.isOnline()) {
                            // 使用 setRotation 锁定视角，而不是 teleport
                            target.setRotation(frozenLoc.getYaw(), frozenLoc.getPitch());
                            // 只锁定位置，不重置视角
                            ...
                            if (currentLoc.getX() != frozenLoc.getX() || 
                                ...
                                Location newLoc = frozenLoc.clone();
                ...
                SchedulerUtils.runLaterOnLocation(target.getLocation(), 180L, () -> {
                    if (target.isOnline()) {
                        // 移除跳跃力度修改器
                        target.getAttribute(Attribute.JUMP_STRENGTH).removeModifier(finalJumpModifierUUID);
                        MessageUtils.sendPlainMessage(target, ChatColor.GREEN + "时间开始流动...");
                    }
                    // 取消视角锁定任务
                    if (lockTask[0] != null) {
                        SchedulerUtils.cancel(lockTask[0]);
                    }
                });
            }
        }
        ...
        // 9秒后提示使用者
        SchedulerUtils.runLaterOnPlayer(player, 180L, () -> {
            MessageUtils.sendPlainMessage(player, ChatColor.GREEN + "时间开始流动...");
        });
        return true;
    }
    ...
    private void useShieldGenerator(Player player) {
        MessageUtils.sendPlainMessage(player, ChatColor.AQUA + "护盾已生成！");
        ...
        // 5秒后移除护盾效果
        SchedulerUtils.runLaterOnLocation(player.getLocation(), 100L, () -> {
            if (player.isOnline()) {
                // 移除击退抗性
                player.getAttribute(Attribute.KNOCKBACK_RESISTANCE).removeModifier(modifierUUID);

                MessageUtils.sendPlainMessage(player, ChatColor.GRAY + "护盾已消失");

...
public class EventSystem {
   ...
   // 断腿事件：记录已经部分恢复的玩家
   private final Set<UUID> brokenLegPartiallyRecovered = new HashSet<>();
   
   // 断腿事件：记录当前处于断腿事件中的玩家（用于取消跳跃事件）
   private final Set<UUID> brokenLegPlayers = new HashSet<>();
   
   // 断腿事件：记录每个玩家的断腿修改器UUID（用于正确移除）
   private final Map<UUID, UUID> brokenLegModifiers = new HashMap<>();
   ...
   /**
    * 检查玩家是否处于断腿事件中（用于取消跳跃）
    * @param player 目标玩家
    * @return 是否在断腿事件中
    */
   public boolean isBrokenLegPlayer(Player player) {
      return brokenLegPlayers.contains(player.getUniqueId());
   }
   ...
   public void start() {
      ...
      if (world != null) {
         this.eventTask = SchedulerUtils.runTimerOnWorld(world, GameConstants.DELAY_IMMEDIATE, GameConstants.TICKS_PER_SECOND, scheduledTask -> {
         ...
         this.durationTask = SchedulerUtils.runTimerOnWorld(world, 1L, 1L, scheduledTask -> {
            if (this.gameManager.getGameStatus() != GameStatus.PLAYING) {
               SchedulerUtils.cancel(scheduledTask);
            } else {
      ...
      if (this.currentEvent != null) {
         switch (this.currentEvent) {
            ...
            case LIGHTNING:
               ...
               break;
            case BROKEN_LEG:
               // 断腿事件不再需要tick检查，跳跃取消在PlayerListener中处理
               break;
            case GREEDY_SLIME:
   ...
   private void endCurrentEvent() {
      if (this.currentEvent != null) {
         ...
         for (Player player : this.getInGamePlayers()) {
            ...
            AttributeInstance jumpAttr = player.getAttribute(Attribute.JUMP_STRENGTH);
            if (jumpAttr != null) {
               // 只移除非断腿事件的跳跃修改器（保留断腿效果的跳跃高度）
               long jumpModCount = jumpAttr.getModifiers().stream().filter(mod ->
                  !mod.getName().startsWith("event_broken_leg") &&
                  Math.abs(mod.getAmount() - -1.0) < 0.001
               ).peek(mod -> {
                  jumpAttr.removeModifier(mod);
                  this.debugLogger.debug("[事件系统] 移除玩家 " + player.getName() + " 的跳跃强度修改器: 名称=" + mod.getName() + ", 数值=" + mod.getAmount());
               }).count();
         ...
         // 断腿事件结束：部分恢复玩家的跳跃能力（只能跳0.5格）
         if (this.currentEvent == EventType.BROKEN_LEG) {
            for (Player player : this.getInGamePlayers()) {
               AttributeInstance jumpAttr = player.getAttribute(Attribute.JUMP_STRENGTH);
               if (jumpAttr != null) {
                  // 使用记录的UUID移除断腿效果
                  UUID modifierUUID = brokenLegModifiers.remove(player.getUniqueId());
                  if (modifierUUID != null) {
                     jumpAttr.removeModifier(modifierUUID);
                     this.plugin.getLogger().info("[断腿事件-结束] 玩家: " + player.getName() + " 移除修改器UUID: " + modifierUUID);
                  }
                  
                  // 添加部分恢复效果（只能跳0.5格高，跳跃力度设为0.3）
                  // 基础跳跃力度0.42 - 0.12 = 0.3（约0.5格高）
                  jumpAttr.addModifier(new AttributeModifier(UUID.randomUUID(), "event_broken_leg_partial", -0.12, Operation.ADD_NUMBER));
                  this.plugin.getLogger().info("[断腿事件-结束] 玩家: " + player.getName() + " 最终跳跃力度: " + jumpAttr.getValue());
               }
               
               // 发送恢复消息
               MessageUtils.sendPlainMessage(player, "§e你的腿恢复了一点，你也许可以跳一点点高");
            }
            
            brokenLegPartiallyRecovered.clear();
   ...
   private void tickFallingAnvil() {
      ...
      if (this.anvilTickCounter % 10 == 0) {
         for (Player player : this.getInGamePlayers()) {
            ...
            SchedulerUtils.runOnLocation(anvilLoc, () -> {
   ...
   private void eventWither() {
      ...
      if (world != null) {
         ...
         SchedulerUtils.runOnLocation(loc, () -> {
   ...
   private void eventFly() {
      ...
      for (Player player : this.getInGamePlayers()) {
         ...
         SchedulerUtils.runOnLocation(loc, () -> {
            ...
            MessageUtils.sendPlainMessage(player, "§b自由的风！获得鞘翅和烟花！");
         });
      }
   }
      ...
      if (this.eventDuration % 10 == 0) {
         for (Player player : this.getInGamePlayers()) {
            ...
            for (int i = 0; i < 5; i++) {
               ...
               SchedulerUtils.runOnLocation(arrowLoc, () -> {
   ...
   private void eventSkyWalker() {
      ...
      for (Player player : this.getInGamePlayers()) {
         ...
         MessageUtils.sendPlainMessage(player, "§e你可以在空中行走了！");
      }
   }
   ...
   private void eventHelloWorld() {
      ...
      for (Player player : this.getInGamePlayers()) {
         ...
         SchedulerUtils.runOnLocation(loc, () -> {
            ...
            MessageUtils.sendPlainMessage(player, "§a你好世界！");
         });
      }
   }
   ...
   private void eventUndead() {
      ...
      for (Player player : this.getInGamePlayers()) {
         ...
         SchedulerUtils.runOnLocation(loc, () -> {
            ...
            MessageUtils.sendPlainMessage(player, "§6不死图腾！+1条命！");
         });
      }
   }
   ...
   private void eventBrokenLeg() {
      ...
      for (Player player : this.getInGamePlayers()) {
         // 添加缓慢效果（持续时间与事件持续时间一致：20秒 = 400 ticks）
         player.addPotionEffect(new PotionEffect(PotionEffectType.SLOWNESS, 400, 2, false, false));
         
         // 将玩家添加到断腿事件集合（用于取消跳跃事件）
         brokenLegPlayers.add(player.getUniqueId());
         
         // 事件期间完全禁止跳跃（将跳跃力度设为0）
         AttributeInstance jumpAttr = player.getAttribute(Attribute.JUMP_STRENGTH);
         if (jumpAttr != null) {
            // 清除之前的断腿效果（使用记录的UUID）
            UUID existingModifierUUID = brokenLegModifiers.remove(player.getUniqueId());
            if (existingModifierUUID != null) {
               jumpAttr.removeModifier(existingModifierUUID);
            }
            
            // 事件期间：使用 ADD_NUMBER 操作，将跳跃力度设为0（完全无法跳跃）
            UUID modifierUUID = UUID.randomUUID();
            AttributeModifier fullModifier = new AttributeModifier(modifierUUID, "event_broken_leg_full", -GameConstants.BASE_JUMP_STRENGTH, Operation.ADD_NUMBER);
            jumpAttr.addModifier(fullModifier);
            brokenLegModifiers.put(player.getUniqueId(), modifierUUID);
            this.plugin.getLogger().info("[断腿事件-开始] 玩家: " + player.getName() + " 添加修改器UUID: " + modifierUUID + ", 当前跳跃力度: " + jumpAttr.getValue());
         }

         MessageUtils.sendPlainMessage(player, "§c你摔断了腿！移动变慢且无法跳跃！");
      }
   }
   ...
   private void eventPunch() {
      ...
      for (Player player : this.getInGamePlayers()) {
         ...
         MessageUtils.sendPlainMessage(player, "§c一击必杀！攻击伤害变为40！");
      }
   }
   ...
   private void eventTouch() {
      ...
      for (Player player : this.getInGamePlayers()) {
         ...
         MessageUtils.sendPlainMessage(player, "§e摸摸！交互距离增加了10格！");
      }
   }
   ...
   private void eventInvExchange() {
      ...
      if (players.size() < 2) {
         ...
         SchedulerUtils.runOnLocation(loc1, () -> {
            inv1.setContents(contents2);
            MessageUtils.sendPlainMessage(p1, "§d你和 " + finalP2.getName() + " 交换了背包！");
            this.plugin.getLogger().info("[事件16-背包交换] " + p1.getName() + " 已获得 " + finalP2.getName() + " 的背包");
         });
         Location loc2 = p2.getLocation();
         SchedulerUtils.runOnLocation(loc2, () -> {
            inv2.setContents(contents1);
            MessageUtils.sendPlainMessage(finalP2, "§d你和 " + p1.getName() + " 交换了背包！");
   ...
   private void eventKingGame() {
      ...
      if (players.isEmpty()) {
         ...
         SchedulerUtils.runOnLocation(kingLoc, () -> {
         ...
         for (Player player : players) {
            if (player != this.currentKing) {
               rebelCount++;
               Location loc = player.getLocation();
               SchedulerUtils.runOnLocation(loc, () -> player.getInventory().addItem(new ItemStack[]{new ItemStack(Material.STONE, 16)}));
            }
         }
   ...
   private void eventLuckyDoll() {
      ...
      DollEntry[] dolls = new DollEntry[]{
         new DollEntry("Tartgralia", "破绽，稍纵即逝！"),
         new DollEntry("Dougllase", "祝你好运"),
         new DollEntry("cxnaive", "哈气！"),
         new DollEntry("CC_Heart", "请为我充值灵感"),
         new DollEntry("MaoBing_OVO", "别视奸我"),
         new DollEntry("chenaojiao_OVO", "这家伙是谁？"),
         new DollEntry("moyu3gz", "便是一场梦，总该醒来了"),
         new DollEntry("xingjianya", "星见雅最摸鱼"),
         new DollEntry("Xyue_UE", "服务器急需免费宣传片"),
         new DollEntry("sunxiaochuan55", "扎扎师。无敌？！嗯。兑！？")
      };
      ...
      for (Player player : this.getInGamePlayers()) {
         ...
         SchedulerUtils.runOnLocation(loc, () -> {
            ...
            this.plugin.getLogger().info("[事件18-幸运玩偶] 玩家: " + player.getName() + " 获得幸运玩偶 " + dollEntry.playerId);
            MessageUtils.sendPlainMessage(player, "§a幸运降临！你获得了幸运玩偶 §b" + dollEntry.playerId);
         });
      }
   }
   ...
   private void eventHungry() {
      ...
      for (Player player : this.getInGamePlayers()) {
         ...
         SchedulerUtils.runOnLocation(loc, () -> {
            ...
            MessageUtils.sendPlainMessage(player, "§c饿啊饿啊！饥饿效果！");
         });
      }
   }
   ...
   private void eventBlack() {
      ...
      for (Player player : this.getInGamePlayers()) {
         ...
         SchedulerUtils.runOnLocation(loc, () -> {
            ...
            MessageUtils.sendPlainMessage(player, "§0真的黑！");
         });
      }
   }
   ...
   private void eventSpeed() {
      ...
      for (Player player : this.getInGamePlayers()) {
         ...
         MessageUtils.sendPlainMessage(player, "§b♿冲刺冲刺♿");
      }
   }
   ...
   private void eventMini() {
      ...
      for (Player player : this.getInGamePlayers()) {
         ...
         MessageUtils.sendPlainMessage(player, "§a> <");
      }
   }
   ...
   private void eventHuge() {
      ...
      for (Player player : this.getInGamePlayers()) {
         ...
         MessageUtils.sendPlainMessage(player, "§c= =");
      }
   }
   ...
   private void eventNuclear() {
      ...
      for (Player player : this.getInGamePlayers()) {
         ...
         SchedulerUtils.runOnLocation(loc, () -> {
            ...
            MessageUtils.sendPlainMessage(player, "§c核电，轻而易举！");
         });
      }
   }
   ...
   private void processSlimeEat(Slime slime, World world) {
      ...
      for (Entity nearby : world.getNearbyEntities(slimeLoc, 3.0, 3.0, 3.0)) {
         if (nearby instanceof Item item) {
            ...
            SchedulerUtils.runOnLocation(itemLoc, () -> {
               if (!item.isDead()) {
                  item.remove();
               }
            });
            ...
            if (eatCount >= 3) {
               ...
               if (currentSize < 10) {
                  ...
                  SchedulerUtils.runOnLocation(slimeLoc, () -> {
                     if (!slime.isDead()) {
   ...
   private void eventLocationExchange() {
      ...
      if (players.size() < 2) {
         ...
         SchedulerUtils.runOnLocation(loc1, () -> p1.teleportAsync(loc2).thenAccept(success -> {
               if (success) {
         ...
         SchedulerUtils.runOnLocation(loc2, () -> finalP2.teleportAsync(loc1)
               .thenAccept(
                  success -> {
                     if (success) {
            ...
            for (int x = -range; x <= range; x++) {
               for (int z = -range; z <= range; z++) {
                  ...
                  if (loc.getBlock().getType() == Material.AIR) {
                     SchedulerUtils.runOnLocation(loc, () -> loc.getBlock().setType(Material.LAVA));
                  }
               }
            }
         }
            ...
            for (int x = -range; x <= range; x++) {
               for (int z = -range; z <= range; z++) {
                  ...
                  if (loc.getBlock().getType() == Material.LAVA) {
                     SchedulerUtils.runOnLocation(loc, () -> loc.getBlock().setType(Material.AIR));
                  }
               }
            }
         }
         ...
         for (Player player : this.getInGamePlayers()) {
            ...
            if (canSeeSky && isDaytime && !isStormy) {
               SchedulerUtils.runOnLocation(loc, () -> {
                  ...
                  MessageUtils.sendPlainMessage(player, "§c阳光太强烈了！你着火了！");
               });
            }
         }
      }
   ...
   private void eventAlwaysExplode() {
      ...
      for (Player player : this.getInGamePlayers()) {
         ...
         SchedulerUtils.runOnLocation(loc, () -> {
            ...
            MessageUtils.sendPlainMessage(player, "§c§lBOOM！§7你的装备快坏了！");
         });
      }
   }

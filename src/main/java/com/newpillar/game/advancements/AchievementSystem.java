...
public class AchievementSystem {
    ...
    public void addKill(Player player) {
        ...
        // 异步保存到数据库
        saveStatsToDatabase(uuid, player.getName());
     ...
     * 保存玩家统计到数据库（异步执行）
     */
    private void saveStatsToDatabase(UUID uuid, String playerName) {
        if (plugin.getDatabaseManager() != null) {
            int kills = playerKills.getOrDefault(uuid, 0);
            int wins = playerWins.getOrDefault(uuid, 0);
            int deaths = playerDeaths.getOrDefault(uuid, 0);

            // 异步保存到数据库
            SchedulerUtils.runAsync(() -> {
                plugin.getDatabaseManager().saveStatistics(uuid, playerName, kills, deaths, wins,
    ...
    private void saveData() {
        // 成就数据现在只保存到数据库，不再保存到配置文件
        // 数据库保存由 grantAchievement 和 incrementKills/incrementWins 方法直接调用
        // 此方法保留用于兼容性和未来扩展
    }

...
public class StatisticsSystem {
    ...
    private PlayerStatistics getOrCreateStats(Player player) {
    ...
    public void recordKill(Player killer, Player victim) {
        PlayerStatistics stats = getOrCreateStats(killer);
        stats.kills++;
        stats.totalKills++;
    ...
    public void loadPlayerData(UUID uuid) {
        ...
        stats.totalMobKills = data.totalMobKills();
    ...
    public void showLeaderboard(Player player, String type) {
        ...
        for (int i = 0; i < Math.min(10, sorted.size()); i++) {
            ...
            // 优先使用记录的 playerName，如果为空则尝试获取在线玩家名字，最后显示"未知玩家"
            String name = stats.getPlayerName();
            if (name == null || name.isEmpty()) {
                name = target != null ? target.getName() : "未知玩家";
            }
    ...
    private void loadData() {
        ...
        if (db != null) {
            ...
            for (Map.Entry<UUID, DatabaseManager.PlayerStatisticsData> entry : dbStats.entrySet()) {
                ...
                // 从数据库加载玩家名字
                stats.setPlayerName(data.playerName());
        ...
        if (plugin.getConfig().contains("statistics")) {
            for (String uuidStr : plugin.getConfig().getConfigurationSection("statistics").getKeys(false)) {
                ...
                // 加载玩家名字
                stats.setPlayerName(plugin.getConfig().getString(path + ".playerName", null));
                ...
                stats.mobKills = plugin.getConfig().getInt(path + ".mobKills", 0);
                stats.totalMobKills = plugin.getConfig().getInt(path + ".totalMobKills", 0);
        ...
        SchedulerUtils.runAsync(() -> {
            ...
            if (db != null) {
                for (Map.Entry<UUID, PlayerStatistics> entry : playerStats.entrySet()) {
                    UUID uuid = entry.getKey();
                    PlayerStatistics stats = entry.getValue();
                    db.saveStatistics(uuid, stats.getPlayerName(), stats.totalKills, stats.totalDeaths, stats.totalWins,
                        stats.totalGamesPlayed, stats.damageDealt, stats.damageTaken,
                        stats.blocksPlaced, stats.blocksBroken, stats.itemsLooted, stats.mobKills,
                        ...
                        stats.totalItemsLooted, stats.totalMobKills);
                }
            }
            ...
            try {
                for (Map.Entry<UUID, PlayerStatistics> entry : playerStats.entrySet()) {
                    ...
                    // 保存玩家名字
                    if (stats.getPlayerName() != null && !stats.getPlayerName().isEmpty()) {
                        plugin.getConfig().set(path + ".playerName", stats.getPlayerName());
                    }
                    ...
                    // 保存新增统计项
                    plugin.getConfig().set(path + ".mobKills", stats.mobKills);
                    ...
                    plugin.getConfig().set(path + ".totalMobKills", stats.totalMobKills);
                }
    ...
    public void syncPlayerToDatabase(UUID uuid) {
        ...
        if (db != null) {
            db.saveStatistics(uuid, stats.getPlayerName(), stats.totalKills, stats.totalDeaths, stats.totalWins,
                ...
                stats.totalDamageTaken, stats.totalBlocksBroken, stats.totalBlocksPlaced,
                stats.totalItemsLooted, stats.totalMobKills);
            plugin.getLogger().info("玩家 " + (stats.getPlayerName() != null ? stats.getPlayerName() : uuid.toString()) + " 的统计信息已同步到数据库");
        }
    ...
    public static class PlayerStatistics {
        ...
        private String playerName = null;  // 玩家名字
        ...
        public int totalMobKills = 0;         // 总生物击杀

...
public class RuleSystem implements Listener {
    ...
    private void startPartnerTask() {
        ...
        partnerTask = SchedulerUtils.runTimerOnWorld(world, 20L, 20L, task -> {
    ...
    public void resetPlayerAttributes(Player player) {
        ...
        if (movementSpeed != null) {
            ...
            movementSpeed.setBaseValue(GameConstants.BASE_MOVEMENT_SPEED);
        }
        ...
        if (jumpStrength != null) {
            ...
            jumpStrength.setBaseValue(GameConstants.BASE_JUMP_STRENGTH);
        }

        // 重置重力（月球地图低重力效果）
        AttributeInstance gravityAttr = player.getAttribute(Attribute.GRAVITY);
        if (gravityAttr != null) {
            gravityAttr.setBaseValue(GameConstants.BASE_GRAVITY); // 默认重力值
        }

        // 重置安全摔落距离（月球地图修改过）
        AttributeInstance safeFallAttr = player.getAttribute(Attribute.SAFE_FALL_DISTANCE);
        if (safeFallAttr != null) {
            safeFallAttr.setBaseValue(GameConstants.BASE_SAFE_FALL_DISTANCE); // 默认安全摔落距离
        }

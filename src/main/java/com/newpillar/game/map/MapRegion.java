...
import com.newpillar.game.PillarManager;
...
public class MapRegion {
      ...
      if (world != null) {
         ...
         for (int x = startX; x <= endX; x += chunkSize) {
            for (int z = startZ; z <= endZ; z += chunkSize) {
               int chunkX = x;
               int chunkZ = z;
               SchedulerUtils.runOnChunk(world, chunkX, chunkZ, () -> {
   ...
   public void createVirtualPlayers(int count, int totalPlayers) {
      ...
      if (world != null) {
         ...
         for (int i = 0; i < count; i++) {
            ...
            SchedulerUtils.runOnLocation(cageLoc, () -> {
   ...
   private void spawnTestMarkerParticles(Location loc) {
      ...
      if (world != null) {
         SchedulerUtils.runTimerOnLocation(loc, 1L, 20L, task -> {
   ...
   public void clearVirtualPlayers() {
      ...
      for (ArmorStand stand : this.virtualPlayerStands.values()) {
         if (stand != null && !stand.isDead()) {
            SchedulerUtils.runOnEntity(stand, () -> {
               if (!stand.isDead()) {
                  stand.remove();
               }
            });

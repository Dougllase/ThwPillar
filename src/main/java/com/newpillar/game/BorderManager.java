...
public class BorderManager {
    ...
    // 配置参数
    private static final double INITIAL_SIZE = 60.0;  // 初始60格
    private static final double MIN_SIZE = 10.0;      // 最小10格，进入平台崩溃
    ...
    private void calculateShrinkParameters() {
        ...
        // 计算总收缩量
        double totalShrink = INITIAL_SIZE - MIN_SIZE; // 60 - 10 = 50格
        
        // 计算每轮收缩大小
        this.shrinkAmount = totalShrink / targetRounds;
        
        // 计算每轮间隔（包括收缩动画时间）
        // 总时长 = 首次等待 + (轮数-1) * 间隔 + 最后一轮动画时间
        // 正确公式：间隔 = (总时长 - 所有动画时间) / 轮数
        int totalAnimationTime = (int)SHRINK_DURATION * targetRounds; // 所有轮次的动画总时间
        this.shrinkInterval = (totalDurationSeconds - totalAnimationTime) / targetRounds;
    ...
    public void startBorderShrink(World world) {
        ...
        this.borderCountdownTask = SchedulerUtils.runTimerOnLocation(centerLoc, 20L, 20L,
            scheduledTask -> {
    ...
    private void runBorderShrinkCycle(World world) {
        ...
        if (currentSize > MIN_SIZE) {
            ...
            // 如果已经达到最小值，不再设置下一轮倒计时
            final int waitTime;
            if (newSize <= MIN_SIZE) {
                debugLogger.debug("[BorderManager] 已达到最小值，等待动画完成后触发崩溃");
                // 等待收缩动画完成后再检查是否触发崩溃
                waitTime = (int) SHRINK_DURATION + 2; // 动画时间 + 2秒缓冲
                this.borderShrinkCountdown = waitTime;
            } else {
                // 正常设置下一轮倒计时
                waitTime = shrinkInterval;
                this.borderShrinkCountdown = waitTime;
                debugLogger.debug("[BorderManager] 设置下一轮倒计时: " + shrinkInterval + " 秒");
            }
